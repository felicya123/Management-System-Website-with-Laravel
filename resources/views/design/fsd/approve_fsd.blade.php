...
@foreach($wb as $a)
@if(Auth::user()->division == "BO" && $a->description == "Waiting for FSD to be approved by Business Owner")
<form method="POST" action="{{url('/work-bo/design/approve-fsd-bo-store/'.$id->id)}}" enctype="multipart/form-data">
@elseif(Auth::user()->division == "IT_AppsManager" && $a->description == "Waiting for FSD to be approved by IT Apps Manager")
<form method="POST" action="{{url('/work-am/design/approve-fsd-am-store/'.$id->id)}}" enctype="multipart/form-data">
@elseif(Auth::user()->division == "IT_Owner" && $a->description == "Waiting for FSD to be approved by IT Owner")
<form method="POST" action="{{url('/work-owner/design/approve-fsd-owner-store/'.$id->id)}}" enctype="multipart/form-data">
@endif
@endforeach
        ...
        <div class="form-group row" id="ifApproveLabel" style="display:none;">
            ...
            <div class="form-group row" id="ifRejectLabel" style="display:none;">
                <label for="title_itsr" class="col-sm-2 col-form-label">Assign To</label>
                    <div class="col-sm-10">
                            @foreach($reject as $a)
                                <input type="text" name="assign_to" id="ifReject" style="display:none;" class="form-control"value="{{$a->fullname}}" readonly>
                            @break
                                @endforeach
                    </div>
            </div>
