...
@foreach($wb as $a)
@if(Auth::user()->division == "IT_AppsManager" && $a->description == "Waiting for TSD to be approved by IT Apps Manager")
<form method="POST" action="{{url('/work-am/design/approve-tsd-am-store/'.$id->id)}}" enctype="multipart/form-data">
@elseif(Auth::user()->division == "IT_Owner" && $a->description == "Waiting for TSD to be approved by IT Owner")
<form method="POST" action="{{url('/work-owner/design/approve-tsd-owner-store/'.$id->id)}}" enctype="multipart/form-data">
@endif
@endforeach
