...
@foreach($wb as $a)
@if(Auth::user()->division == "IT_SA" &&  $a->description == "Waiting for TSD to be reviewed by IT_SA")
<form method="POST" action="{{url('/work-sa/design/review-tsd-sa-store/'.$id->id)}}" enctype="multipart/form-data">
@elseif(Auth::user()->division == "IT_SA" && $a->description == "Waiting for TSD to be reviewed by Impacted IT SA" )
<form method="POST" action="{{url('/work-sa/design/review-tsd-sa-impacted-store/'.$id->id)}}" enctype="multipart/form-data">
@elseif(Auth::user()->division == "IT_Security" && $a->description == "Waiting for TSD to be reviewed by IT Security")
<form method="POST" action="{{url('/work-security/design/review-tsd-security-store/'.$id->id)}}" enctype="multipart/form-data">
@elseif(Auth::user()->division == "IT_Infra" && $a->description == "Waiting for TSD to be reviewed by IT Infra")
<form method="POST" action="{{url('/work-infra/design/review-tsd-infra-store/'.$id->id)}}" enctype="multipart/form-data">
@endif
@endforeach
<div id="Approval" class="tabcontent">
 @csrf
    ...
    <div class="">
        ...
        @if(Auth::user()->division == "IT_SA" && $row->description == "Waiting for TSD to be reviewed by IT_SA" && $assess->impacted_app == 0)
                    ...
                    <div class="col-sm-10">
                        <select name="assign_to"  class="form-control" id="ifApprove" style="display:none;"disabled>
                            ...
                            @foreach($reject as $re)
                                <input type="input" class="form-control" name="assign_to" id="ifReject" style="display:none;" value="{{$re->fullname}}" readonly>
                            @break
                            @endforeach
                    </div>
        </div>
        @elseif(Auth::user()->division == "IT_SA" && $row->description == "Waiting for TSD to be reviewed by IT_SA" && $assess->impacted_app == 1)
        <div class="form-group row" id="ifApproveLabel" style="display:none;">
                <label for="title_itsr" class="col-sm-2 col-form-label">Assign To</label>
                    <div class="col-sm-10">
                        <select name="assign_to"  class="form-control" id="ifApprove" style="display:none;"disabled>
                            @foreach($users as $user)
                            @if($user->division == "IT_SA" && ($user->fullname == $assess->pic_impacted_app1 || $user->fullname == $assess->pic_impacted_app2 || $user->fullname == $assess->pic_impacted_app3) )
                                @if(Auth::user()->fullname != $user->fullname)
                                    <option value="{{$user->fullname}}">{{$user->fullname}}</option>
                                @endif
                                @endif
                            @endforeach
        ...
        @elseif(Auth::user()->division == "IT_SA"  && $assess->impacted_app != null && $assess->total_impacted_app > 1)
        <div class="form-group row" id="ifApproveLabel" style="display:none;">
                    <label for="title_itsr" class="col-sm-2 col-form-label">Assign To</label>
                    <div class="col-sm-10">
                        <select name="assign_to"  class="form-control" id="ifApprove" style="display:none;"disabled>
        ...
        @elseif(Auth::user()->division == "IT_Security")
            ...
            @elseif(Auth::user()->division == "IT_Infra")
            <div class="form-group row" id="ifApproveLabel" style="display:none;">
                    <label for="title_itsr" class="col-sm-2 col-form-label">Assign To</label>
                    <div class="col-sm-10">
                        <select name="assign_to" class="form-control" id="ifApprove" style="display:none;"disabled>
        ...
        <div class="form-group row">
        <label class="col-sm-2 col-form-label" for="tsd_sa_review_file">Choose file</label>
            <div class="custom-file custom-file col-sm-10">
                <input type="file" class="custom-file-input" id="tsd_sa_review_file" name="tsd_sa_review_file" onchange="return fileValidation()" required>
                <label class="custom-file-label" for="tsd_sa_review_file">Choose file</label>
        ...
        <script>
            $('#tsd_sa_review_file').on('change',function(){
            ...
            function fileValidation() { 
                var fileInput =  
                    document.getElementById('tsd_sa_review_file'); 
        ...
        @elseif(Auth::user()->division == "IT_Infra")
        <div class="form-group row">
        <label class="col-sm-2 col-form-label" for="tsd_infra_review_file">Choose file</label>
            <div class="custom-file custom-file col-sm-10">
                <input type="file" class="custom-file-input" id="tsd_infra_review_file" name="tsd_infra_review_file" onchange="return fileValidation2()" required>
                <label class="custom-file-label" for="tsd_infra_review_file">Choose file</label>
                <i>File extention must be in .pdf format</i>
            </div>
        </div>
        <script>
            $('#tsd_infra_review_file').on('change',function(){
                var fileName = $(this).val();
                var cleanFileName = fileName.replace('C:\\fakepath\\', " ");
                $(this).next('.custom-file-label').html(cleanFileName);
            })
            function fileValidation2() { 
                var fileInput =  
                    document.getElementById('tsd_infra_review_file'); 
        ...
        @elseif(Auth::user()->division == "IT_Security")
        <div class="form-group row">
        <label class="col-sm-2 col-form-label" for="tsd_security_review_file">Choose file</label>
            <div class="custom-file custom-file col-sm-10">
                <input type="file" class="custom-file-input" id="tsd_security_review_file" name="tsd_security_review_file" onchange="return fileValidation3()" required>
                <label class="custom-file-label" for="tsd_security_review_file">Choose file</label>
                <i>File extention must be in .pdf format</i>
            </div>
        </div>
        <script>
            $('#tsd_security_review_file').on('change',function(){
                var fileName = $(this).val();
                var cleanFileName = fileName.replace('C:\\fakepath\\', " ");
                $(this).next('.custom-file-label').html(cleanFileName);
            })
            function fileValidation3() { 
                var fileInput =  
                    document.getElementById('tsd_security_review_file'); 
