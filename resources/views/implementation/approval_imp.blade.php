...
@foreach($wb as $a)
@if(Auth::user()->division == "BO" && $a->description == "Waiting for Memo Deploy to be approved by Business Owner")
          <h2>Memo Deploy Approval Form</h2>
@elseif(Auth::user()->division == "BO" && $a->description == "Waiting for PAT to be approve by Business Owner")
          <h2>PAT Execution Approval Form</h2>
@endif
@endforeach
</div>
...
@foreach($wb as $a)
@if(Auth::user()->division == "BO" && $a->description == "Waiting for PAT to be approve by Business Owner")
        <div class="form-group row">
...
@foreach($wb as $a)
@if(Auth::user()->division == "BO" && $a->description == "Waiting for Memo Deploy to be approved by Business Owner")
<form method="POST" action="{{url('/work-bo/implementation/approve-memodeploy-bo-store/'.$id->id)}}" enctype="multipart/form-data">
@elseif(Auth::user()->division == "BO" && $a->description == "Waiting for PAT to be approve by Business Owner")
<form method="POST" action="{{url('/work-bo/implementation/approve-pat-bo-store/'.$id->id)}}" enctype="multipart/form-data">
@endif
@endforeach
<div id="Approval" class="tabcontent">
 @csrf
    ...
    <div class="">
        ...
        @elseif(Auth::user()->division == "BO" && $a->description == "Waiting for PAT to be approve by Business Owner")
        <div class="form-group row">
            <label for="status_itsr_ba" class="col-sm-2 col-form-label">Status PAT Approval</label>
            <div class="col-sm-10">
                <div class="col-sm">
                    <div class="btn-group">
                        <input type="radio" name="status_itsr" value="PAT Success" required>PAT Success
                        <input type="radio" name="status_itsr" value="PAT Fail" required>PAT Fail
                    </div>                    
                </div>
            </div>
        </div> 
