...
<div class="section-title">
          <h2>Development Approval Form</h2>
</div>
...
<h3>Document Attachment</h3>
    @foreach($data as $row)
            ...
            <div class="form-group row">
            <label class="col-sm-2 col-form-label" for="fsd_file">Development File (Week-{{$row->week}})</label>
                <div class="custom-file custom-file col-sm-10" >
    ...
    @foreach($data as $row)
        ...
        <label class="col-sm-2 col-form-label" for="fsd_file">Development SA File</label>
            <div class="custom-file custom-file col-sm-10">
                <a href="/files/dev/sa/{{$itsr->itsr_no}}" target="_blank"><button class="btn">view</button></a>
            </div>
        </div>
        @endif
        @break
...
<form method="POST" action="{{url('/work-am/dev/approve-dev-am-store/'.$id->id)}}" enctype="multipart/form-data">
<div id="Approval" class="tabcontent">
 @csrf
    ...
    <div class="">
        ...
        <div class="form-group row" id="assigntoLabel" style="display:none;">
                    <label for="title_itsr" class="col-sm-2 col-form-label">Assign To</label>
                    <div class="col-sm-10">
                        <select name="assign_to"  class="form-control"  id="assignto_ba" style="display:none;" disabled>
                        ...
                        <select name="assign_to"  class="form-control"  id="assignto_developer" style="display:none;" disabled readonly>
                            @foreach($users as $user)
...
function approvereject() {
    if (document.getElementById('approveCheck').checked) {
        document.getElementById('assigntoLabel').style.display = '';
        document.getElementById('assignto_ba').style.display = '';
        document.getElementById('assignto_developer').style.display = 'none';
        document.getElementById('assignto_developer').disabled = true;
        document.getElementById('assignto_ba').disabled = false;
    }
    else {
        document.getElementById('assigntoLabel').style.display = '';
        document.getElementById('assignto_ba').style.display = 'none';
        document.getElementById('assignto_developer').style.display = '';
        document.getElementById('assignto_developer').disabled = false;
        document.getElementById('assignto_ba').disabled = true;
    } 
