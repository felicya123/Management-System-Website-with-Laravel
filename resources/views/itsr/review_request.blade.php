...
<form action="{{route('itsr.reviewRequest_store')}}" method="POST" enctype="multipart/form-data" >
    @csrf
    ...
    <div class="">
        ...
        <div class="form-group row">
            ...
            <div id="ifYes" style="display:none" class="form-group row" >
                <label for="pre_review_status" class="col-sm-2 col-form-label">Pre-Review</label>
                <div class="col-sm-10">
                    <div class="col-sm">
                        <div class="btn-group">
                            <input type="radio" id="pre_review_status" name="pre_review_status" value="1" required disabled>Yes
                            <input type="radio" id="pre_review_status2"name="pre_review_status" value="0" disabled>No    
                        </div>
                    </div>
                </div>
            </div>  

            <div class="form-group row" id="assignto_label_reject" style="display:none;">
                <label for="title_itsr" class="col-sm-2 col-form-label">Assign To</label>
                <div class="col-sm-10">
                @foreach($users as $user)
                        @if($itsr->create_by == $user->user_id)
                <input type="text" name="assignto_reject"  id="assignto_reject" class="form-control" value="{{$user->fullname}}" readonly>
                        @endif
                @endforeach
                </div>
            </div>
...
function approvereject() {
    if (document.getElementById('yesCheck').checked) {
        ...
        document.getElementById('assignto_label_reject').style.display= 'none';
        document.getElementById('assignto_reject').style.display = 'none';
        document.getElementById('pre_review_status').disabled = false;
        document.getElementById('pre_review_status2').disabled = false;
        
    }
    else {
        document.getElementById('ifYes').style.display = 'none';
        document.getElementById('ifYes').disabled = true;
        document.getElementById('assignto_label_reject').style.display= '';
        document.getElementById('assignto_reject').style.display = '';
        document.getElementById('pre_review_status').disabled = true;
        document.getElementById('pre_review_status2').disabled = true;
    }
