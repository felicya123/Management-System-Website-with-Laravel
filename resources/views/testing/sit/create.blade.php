...
<!--Submission-->
@foreach($wb as $a)
@if(Auth::user()->division == "IT_BA" &&  $a->description == "Waiting for SIT Test Result to be submitted by IT BA")
<form method="POST" action="{{url('/work-ba/testing/create-sit-tr-store/'.$id->id)}}" enctype="multipart/form-data">
@elseif(Auth::user()->division == "IT_BA" &&  $a->description == "Waiting for SIT Test Result to be revised by IT BA")
<form method="POST" action="{{url('/work-ba/testing/create-sit-tr-store/'.$id->id)}}" enctype="multipart/form-data">
@else
<form method="POST" action="{{url('/work-ba/testing/create-sit-tp-store/'.$id->id)}}" enctype="multipart/form-data">
@endif
@endforeach
<div id="SubmissionFSD" class="tabcontent">
    @csrf
        <div class="">
         ...
         <div class="form-group row">
                    <label class="col-sm-2 col-form-label" for="sit_tp_file">Choose file</label>
                    <div class="custom-file custom-file col-sm-10">
                        <input type="file" class="custom-file-input" id="sit_tp_file" name="sit_tp_file" onchange="return fileValidation()" required>
                ...
                <script>
                    $('#sit_tp_file').on('change',function(){
                    ...
                    function fileValidation() { 
                        var fileInput =  
                            document.getElementById('sit_tp_file'); 
