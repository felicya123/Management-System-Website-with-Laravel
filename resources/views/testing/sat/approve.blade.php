...
@foreach($wb as $a)
@if(Auth::user()->division == "IT_SecurityHead" && $a->description == "Waiting for SAT Test Plan to be approved by IT Security Head")
<form method="POST" action="{{url('/work-securityhead/testing/approve-sat-tp-securityhead-store/'.$id->id)}}" enctype="multipart/form-data">
@elseif(Auth::user()->division == "IT_SecurityHead" && $a->description == "Waiting for SAT Test Result to be approved by IT Security Head")
<form method="POST" action="{{url('/work-securityhead/testing/approve-sat-tr-securityhead-store/'.$id->id)}}" enctype="multipart/form-data">
@endif
@endforeach
<div id="Approval" class="tabcontent">
 @csrf
    <div class="">
        ...
        @foreach($wb as $b)
        @if($b->description == "Waiting for SAT Test Plan to be approved by IT Security Head")
            ...
            @elseif($b->description == "Waiting for SAT Test Result to be approved by IT Security Head")
            <div class="form-group row" id="ifApproveLabel" style="display:none;">
                    <label for="title_itsr" class="col-sm-2 col-form-label">Assign To</label>
                    <div class="col-sm-10">
                        <select name="assign_to" class="form-control" id="ifApprove" style="display:none;"disabled>
