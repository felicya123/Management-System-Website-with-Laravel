...
<h3>Document Attachment</h3>
        ...
        @endif
        @if($a->sat_report_file != null)
        <div class="form-group row">
        <label class="col-sm-2 col-form-label" for="inputGroupFile04">SAT Execution Report File</label>
            <div class="custom-file custom-file col-sm-10">
                <a href="/files/sat-report/{{$itsr->itsr_no}}" target="_blank"><button class="btn">view</button></a>
            </div>
        </div>
        @endif
        @if($a->sat_tr_file != null)
        <div class="form-group row">
...
<!--Submission-->
@foreach($wb as $a)
@if(Auth::user()->division == "IT_Security" &&  $a->description == "Waiting for SAT Test Result to be submitted by IT Security")
<form method="POST" action="{{url('/work-security/testing/create-sat-tr-store/'.$id->id)}}" enctype="multipart/form-data">
@elseif(Auth::user()->division == "IT_Security" &&  $a->description == "Waiting for SAT Test Result to be revised by IT Security")
<form method="POST" action="{{url('/work-security/testing/create-sat-tr-store/'.$id->id)}}" enctype="multipart/form-data">
@else
<form method="POST" action="{{url('/work-security/testing/create-sat-tp-store/'.$id->id)}}" enctype="multipart/form-data">
@endif
@endforeach
<div id="SubmissionFSD" class="tabcontent">
    @csrf
        <div class="">
                    ...
                    <div class="col-sm-10">
                        <select name="assign_to"  class="form-control">
                            @foreach($users as $user)
                                @if($user->division == "IT_SecurityHead")
            ...
            <div class="form-group row">
                    <label class="col-sm-2 col-form-label" for="sat_file">Choose file</label>
                    <div class="custom-file custom-file col-sm-10">
                        <input type="file" class="custom-file-input" id="sat_file" name="sat_file" onchange="return fileValidation()" required>
                ...
                <script>
                    ...
                    function fileValidation() { 
                        var fileInput =  
                            document.getElementById('sat_file'); 
