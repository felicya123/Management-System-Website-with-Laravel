...
@foreach($wb as $awb)
@if(Auth::user()->division == "BO" && $a->description == "Waiting for UAT Test Plan to be approved by Business Owner")
<form method="POST" action="{{url('/work-bo/testing/approve-uat-tp-bo-store/'.$id->id)}}" enctype="multipart/form-data">
@elseif(Auth::user()->division == "BO" && $a->description == "Waiting for UAT Test Result to be approved by Business Owner")
<form method="POST" action="{{url('/work-bo/testing/approve-uat-tr-bo-store/'.$id->id)}}" enctype="multipart/form-data">
@endif
@endforeach
