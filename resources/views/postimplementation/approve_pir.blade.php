...
<h3>Document Attachment</h3>
        @if($data->pir_file != NULL)
        <div class="form-group row">
        <label class="col-sm-2 col-form-label" for="inputGroupFile04">PIR File</label>
            <div class="custom-file custom-file col-sm-10">
                <a href="/files/pir/{{$itsr->itsr_no}}" target="_blank"><button class="btn">view</button></a>
            </div>
        </div>
        @endif
</div>
...
@foreach($wb as $a)
@if(Auth::user()->division == "BO" && $a->description == "Waiting for PIR Document to be approved by Business Owner")
<form method="POST" action="{{url('/work-bo/postimplementation/approve-pir-bo-store/'.$id->id)}}" enctype="multipart/form-data">
@elseif(Auth::user()->division == "IT_AppsManager" && $a->description == "Waiting for PIR Document to be approved by IT Apps Manager")
<form method="POST" action="{{url('/work-am/postimplementation/approve-pir-am-store/'.$id->id)}}" enctype="multipart/form-data">
@elseif(Auth::user()->division == "IT_Owner" && $a->description == "Waiting for PIR Document to be approved by IT Owner")
<form method="POST" action="{{url('/work-owner/postimplementation/approve-pir-owner-store/'.$id->id)}}" enctype="multipart/form-data">
@endif
@endforeach
    ...
    <div class="">
        ...
        <div class="form-group row">
            ...
            <div class="col-sm-10">
                <div class="col-sm">
                    <div class="btn-group">
                        <input type="radio" onclick="javascript:approvereject1();"name="status_itsr" value="Approve" id="yesCheck"required>Approve
                        <input type="radio" onclick="javascript:approvereject1();"name="status_itsr" value="Reject" id="noCheck"required>Reject
                    </div>                    
                </div>
            </div>
        </div>   
        ...
        <div class="form-group row" id="ifApproveLabel" style="display:none;">
            ...
            @elseif(Auth::user()->division == "IT_Owner")
            <div class="form-group row" id="ifApproveLabel" style="display:none;">
                    <label for="title_itsr" class="col-sm-2 col-form-label">Assign To</label>
                    <div class="col-sm-10">
                        @foreach($reject as $a)
